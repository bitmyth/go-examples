...
func main() {
	//创建延迟消息
	dm := NewDelayMessage()
	datetime := "2019-11-05 23:00:00"

	startTime, _ := carbon.Parse(TimeFormat, datetime, TIMEZONE)
	//从字符串转为时间戳，第一个参数是格式，第二个是要转换的时间字符串
	//startTime, _ := time.Parse(TimeFormat, datetime)
	...
	err := dm.AddTask(time.Now().Add(time.Second*time.Duration(seconds)), "test3", func(args ...interface{}) {
		...
		cmd := exec.Command("/bin/sh", "-c", `echo "this is my test mail" | mail -s 'mail test' fishis@163.com`)
